...
var SYSTEM_PROMPT = `You are a concise environmental impact analyst. Use specific numbers and be memorable. Output ONLY valid JSON, no markdown, no explanation.`;
  ...
  for (const model of TEXT_MODELS) {
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        if (isQuota) {
          const waitMs = attempt === 1 ? 3e3 : 8e3;
...
async function generateVision(base64Image, mimeType, prompt) {
  const groq = getGroq();
  let lastError;
